...
import com.xs2mobile.buurapp.presentation.presenter.impl.MainPresenterImpl;
...
import com.xs2mobile.buurapp.presentation.view.impl.fragment.BaseMvpFragment;
import com.xs2mobile.buurapp.presentation.view.impl.fragment.InitiativesListFragment;
import com.xs2mobile.buurapp.presentation.view.impl.fragment.MenuFragment;
import com.xs2mobile.buurapp.presentation.view.impl.fragment.NeighboursListFragment;
import com.xs2mobile.buurapp.presentation.view.impl.fragment.ProfileFragment;
...
public class MainActivity extends BaseMvpActivity<MainView, MainPresenter> implements MenuFragment.MenuFragmentListener, MainView {
    ...
    public void onMessageReceived(MessageModel message) {
        if (currentFragment != null) {
            if (currentFragment instanceof InitiativesListFragment) {
                BusProvider.getInstance().post(BusProvider.produceCreateNotificationEvent(message));
            } else if (currentFragment instanceof NeighboursListFragment) {
                BusProvider.getInstance().post(BusProvider.produceRefreshScreenEvent());
    ...
    @Override
    public void goToNeighbourChat(UserModel user, ChatModel chat) {
        Intent intent = new Intent(this, NeighboursChatActivity.class);
    ...
    @Override
    public void goToInitiativeDetail(InitiativeModel model) {
        Intent intent = new Intent(this, InitiativeDetailActivity.class);
        intent.putExtra(NavigationManager.INITIATIVE, model.toJsonString());
        startActivityForResult(intent, 1);
    }

    @Override
    /**
     * This method is call when all necessary data is received
     */
...
//            Crashlytics.logException(e);
    ...
    private void checkNotificationReceived() {

        Bundle extras = getIntent().getExtras();
        if (null != extras) {
            String pushId = extras.getString(NotificationManager.PUSH_UNIQUE_ID_TAG);
            String messageJson = extras.getString(NotificationManager.MESSAGE);
            NotificationManager.NOTIFICATION_KEY notificationKey = (NotificationManager.NOTIFICATION_KEY) extras.get(NotificationManager.PUSH_TAG);
            presenter.onNotificationReceived(pushId, notificationKey, messageJson);
        }
    }
}